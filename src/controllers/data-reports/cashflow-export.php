...
function cashflowBuildPdfHtml(
    ...
    foreach (['PHP', 'USD'] as $currency) {
        ...
        $pages[] = '<section class="report-page">'
            ...
            . '<thead><tr><th rowspan="3">DATE</th><th colspan="8">PARTNER SETTLEMENT DATA</th><th rowspan="3">BANK DEPOSIT</th><th rowspan="3">RUNNING<br>BALANCE</th><th rowspan="3">STATUS</th></tr>'
            ...
            . '<tr class="forwarded"><td class="date">' . cashflowPdfEscape($report['forwarded_date'] ?? '') . '</td>'
            ...
            . '<td>' . cashflowPdfNumber($columnTotals['deposit']) . '</td><td></td><td></td></tr></table>'
    ...
    return '<!doctype html><html><head><meta charset="UTF-8"><style>'
        ...
        . '.date{text-align:left;white-space:nowrap;font-size:5.2pt;letter-spacing:-.08pt}.number{text-align:right;white-space:nowrap}.volume-value{text-align:center}.remarks{text-align:center;white-space:nowrap;font-size:5pt;letter-spacing:-.1pt}.negative{color:#ed2947}.commission td:first-child{text-align:right;font-weight:700;font-style:italic}.forwarded td{font-weight:700}.forwarded .date{font-weight:400;text-align:left}.forwarded .forwarded-label{text-align:left;color:#ed2947}'
...
function cashflowBuildSheet(
    ...
    $sheet->setCellValue('L12', 'STATUS');

    $summary = is_array($report['summary'] ?? null) ? $report['summary'] : [];
    $sheet->mergeCells('B15:J15');
    $sheet->setCellValue('A15', (string) ($report['forwarded_date'] ?? ''));
