<?php
// atinito-insert.php
// Handles duplicate checks, deletes and inserts for ATINITO upload flow

require_once __DIR__ . '/atinito-insert-lib.php';
...
// helpers are provided by atinito-helper.php (atinito_parse_date_claimed, atinito_normalize_amount, ...)
...
try{
    ...
    if($action === 'check'){
        ...
        foreach($pairs as $p){
            ...
            $norm = atinito_parse_date_claimed($rawDate);
            if($norm !== null){
                $stmt = $pdo->prepare('SELECT ccref_no, date_claimed, COUNT(*) as cnt FROM atinito_web_data WHERE ccref_no = ? AND date_claimed = ? GROUP BY ccref_no, date_claimed');
            ...
            if($ts !== false){
                ...
                $stmt2 = $pdo->prepare('SELECT ccref_no, date_claimed, COUNT(*) as cnt FROM atinito_web_data WHERE ccref_no = ? AND DATE(date_claimed) = ? GROUP BY ccref_no, date_claimed');
            ...
            $stmt3 = $pdo->prepare('SELECT ccref_no, date_claimed, COUNT(*) as cnt FROM atinito_web_data WHERE ccref_no = ? GROUP BY ccref_no, date_claimed');
            $stmt3->execute([$cc]);
            $r3 = $stmt3->fetchAll();
            foreach($r3 as $ra){
                if(isset($ra['cnt']) && (int)$ra['cnt'] > 0){
    ...
    if($action === 'delete'){
        ...
        foreach(array_chunk($pairs, 5000) as $chunk){
            ...
            $sql = 'DELETE FROM atinito_web_data WHERE (ccref_no, date_claimed) IN (' . implode(',', $place) . ')';
    ...
    // partner duplicate check: reference_no + date against atinito_partner_data
    if($action === 'check_partner'){
        ...
        foreach($pairs as $p){
            ...
            if($dateOnly !== null){
                $stmt = $pdo->prepare('SELECT reference_no, date, COUNT(*) as cnt FROM atinito_partner_data WHERE reference_no = ? AND DATE(`date`) = ? GROUP BY reference_no, date');
            ...
            $stmt3 = $pdo->prepare('SELECT reference_no, date, COUNT(*) as cnt FROM atinito_partner_data WHERE reference_no = ? GROUP BY reference_no, date');
            $stmt3->execute([$ref]);
            $r3 = $stmt3->fetchAll();
            foreach($r3 as $ra){
                if(isset($ra['cnt']) && (int)$ra['cnt'] > 0){
    ...
    if($action === 'delete_partner'){
        ...
        foreach(array_chunk($pairs, 5000) as $chunk){
            ...
            $sql = 'DELETE FROM atinito_partner_data WHERE (reference_no, date) IN (' . implode(',', $place) . ')';
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
            $cnt += $stmt->rowCount();
        }
    ...
    if($action === 'insert_web'){
        $company = isset($data['company']) ? $data['company'] : 'ATINITO';
        $payloads = isset($data['payloads']) && is_array($data['payloads']) ? $data['payloads'] : [];
        $ins = new AtinitoInsert();
