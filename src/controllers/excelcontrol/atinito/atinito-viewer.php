...
.atinito-viewer{padding:14px 10px;color:#1f2937}
.atinito-viewer h3{display:block;margin:0 0 6px 0;color:#1f2937}
.atinito-view-meta{display:flex;gap:12px;align-items:center;margin-bottom:10px}
.atinito-viewer #atinitoViewerContainer{overflow:auto;max-height:78vh}
.atinito-view-table{border-collapse:collapse;width:auto;min-width:1400px;table-layout:fixed;font-size:13px;border-spacing:0}
.atinito-view-table th,
.atinito-view-table td{
...
.atinito-view-table th:first-child,
.atinito-view-table td:first-child{
    position:sticky;
    left:0;
    z-index:7;
    background:#fff;
}
.atinito-view-table tbody tr:nth-child(even) td{background:rgba(240,244,247,0.6)}
.atinito-view-table tbody tr.search-match td{ background: #fff7ed; }
.atinito-viewer input#atinitoViewerSearch{padding:8px 10px;border-radius:8px;border:1px solid #e5e7eb}
.atinito-viewer .viewer-title{font-weight:600;color:#1f2937}
/* Reserve space on the right of the header for a modal close button */
.atinito-recon-modal__head{padding-right:56px}
@media (max-width:720px){
    .atinito-recon-modal__head{padding-right:12px}
    .atinito-viewer input#atinitoViewerSearch{max-width:180px}
}
</style>
<div class="atinito-viewer" style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;">
    <div class="atinito-recon-modal__head" style="border-bottom:0;margin-bottom:8px;padding-bottom:6px;display:flex;align-items:center;justify-content:space-between">
        <div>
            <h3 style="margin:0;font-size:1.05rem;color:#1f2937;font-weight:700;">
                <?php echo (($partnerName && $partnerName !== '') ? $partnerName : 'ATINITO') . ' - ' . $dateStr; ?>
            </h3>
            ...
            <div style="font-size:0.95rem;color:#666;flex:0 0 auto">Rows: <strong id="atinitoViewerCount"><?php echo number_format(count($rows)); ?></strong></div>
            <input id="atinitoViewerSearch" placeholder="Search" style="flex:1;min-width:0;max-width:360px;padding:8px 10px;border:1px solid #ddd;border-radius:8px" />
            <button id="atinitoViewerSearchBtn" class="material-btn" style="white-space:nowrap">Search</button>
        </div>
    </div>
    <div style="overflow:auto; max-height:78vh;" id="atinitoViewerContainer">
        <table id="atinitoViewerTable" class="atinito-view-table" border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:auto;min-width:1400px">
            <thead>
                <tr>
<?php
...
<?php
    ...
    ;(function(){
        ...
        const container = document.getElementById('atinitoViewerContainer');
        const table = document.getElementById('atinitoViewerTable');
        const rows = container ? container.querySelectorAll('tbody tr') : [];
        const countEl = document.getElementById('atinitoViewerCount');
        const search = document.getElementById('atinitoViewerSearch');
        const btn = document.getElementById('atinitoViewerSearchBtn');
            ...
            if(q === ''){
                ...
                for(let i=0;i<rows.length;i++){
                    ...
                    if(VIEW_TYPE === 'web'){
                        ...
                        const NO_RECORD_ID = 'atinitoNoRecord';
