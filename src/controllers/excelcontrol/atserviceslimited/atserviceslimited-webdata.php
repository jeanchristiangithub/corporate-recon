...
use PhpOffice\PhpSpreadsheet\Shared\Date as ExcelDate;
 ...
 try{
    ...
    // helper to normalize header text (strip BOMs, NBSPs, collapse whitespace, uppercase)
    ...
    // required columns (same as MBTC web format)
    $required = [
    ...
    // search the first 12 rows for a header row that contains our required columns
    $headers = [];
    $headerRow = null;
    ...
    for($rowCheck=1;$rowCheck<=$maxHeaderSearch;$rowCheck++){
        ...
        // if at least 2 of required headers found treat as header row
        if($found >= 2){
            $headers = $tmp;
            $headerRow = $rowCheck;
            break;
    ...
    // iterate rows starting after header row
    for($r=$headerRow+1;$r<=$highestRow;$r++){
        ...
        // capture date if not yet captured
