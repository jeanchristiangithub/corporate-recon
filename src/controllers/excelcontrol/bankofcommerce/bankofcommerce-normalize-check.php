<?php
// bankofcommerce-normalize-check.php
...
try{
    ...
    if(isset($data['payloads']) && is_array($data['payloads'])){
        $pairs = bankofcommerce_build_pairs_from_payloads($data['payloads']);
    } elseif(isset($data['pairs']) && is_array($data['pairs'])){
        // normalize incoming pairs
        foreach($data['pairs'] as $p){
            $cc = isset($p['ccref_no']) ? trim((string)$p['ccref_no']) : '';
            $raw = $p['date_claimed'] ?? ($p['raw_date'] ?? '');
            $pairs[] = ['ccref_no'=>$cc, 'raw_date'=>$raw, 'date_claimed'=>bankofcommerce_parse_date_claimed($raw)];
        }
    }

    // return normalized pairs and duplicates
    $normalized = [];
    foreach($pairs as $p){
        $normalized[] = ['ccref_no'=>$p['ccref_no'],'raw_date'=>$p['raw_date'] ?? null,'normalized'=>$p['date_claimed'] ?? null];
    }

    // perform duplicate detection (reuse tolerant approach)
