...
.bankofcommerce-viewer{padding:14px 10px;color:#1f2937}
.bankofcommerce-viewer h3{display:block;margin:0 0 6px 0;color:#1f2937}
.bankofcommerce-view-meta{display:flex;gap:12px;align-items:center;margin-bottom:10px}
.bankofcommerce-viewer #bankofcommerceViewerContainer{overflow:auto;max-height:78vh}
.bankofcommerce-view-table{border-collapse:collapse;width:auto;min-width:1400px;table-layout:fixed;font-size:13px;border-spacing:0}
.bankofcommerce-view-table th,
.bankofcommerce-view-table td{
...
.bankofcommerce-view-table thead th{
    position:sticky;
    top:0;
    z-index:6;
    background:#f9fafb;
...
.bankofcommerce-view-table th:first-child,
.bankofcommerce-view-table td:first-child{
    position:sticky;
    left:0;
    z-index:7;
    background:#fff;
}
.bankofcommerce-view-table tbody tr:nth-child(even) td{background:rgba(240,244,247,0.6)}
.bankofcommerce-view-table tbody tr.search-match td{ background: #fff7ed; }
.bankofcommerce-viewer input#bankofcommerceViewerSearch{padding:8px 10px;border-radius:8px;border:1px solid #e5e7eb}
.bankofcommerce-viewer .viewer-title{font-weight:600;color:#1f2937}
/* Reserve space on the right of the header for a modal close button */
.bankofcommerce-recon-modal__head{padding-right:56px}
@media (max-width:720px){
    .bankofcommerce-recon-modal__head{padding-right:12px}
    .bankofcommerce-viewer input#bankofcommerceViewerSearch{max-width:180px}
}
</style>
<div class="bankofcommerce-viewer" style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;">
    <div class="bankofcommerce-recon-modal__head" style="border-bottom:0;margin-bottom:8px;padding-bottom:6px;display:flex;align-items:center;justify-content:space-between">
        <div>
            <h3 style="margin:0;font-size:1.05rem;color:#1f2937;font-weight:700;">
                <?php echo (($partnerName && $partnerName !== '') ? $partnerName : 'BANKOFCOMMERCE') . ' - ' . $dateStr; ?>
            </h3>
            ...
            <div style="font-size:0.95rem;color:#666;flex:0 0 auto">Rows: <strong id="bankofcommerceViewerCount"><?php echo number_format(count($rows)); ?></strong></div>
            <input id="bankofcommerceViewerSearch" placeholder="Search" style="flex:1;min-width:0;max-width:360px;padding:8px 10px;border:1px solid #ddd;border-radius:8px" />
            <button id="bankofcommerceViewerSearchBtn" class="material-btn" style="white-space:nowrap">Search</button>
        </div>
    </div>
    <div style="overflow:auto; max-height:78vh;" id="bankofcommerceViewerContainer">
        <table id="bankofcommerceViewerTable" class="bankofcommerce-view-table" border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:auto;min-width:1400px">
            <thead>
                <tr>
<?php
...
<?php
    ...
    ;(function(){
        ...
        const container = document.getElementById('bankofcommerceViewerContainer');
        const table = document.getElementById('bankofcommerceViewerTable');
        const rows = container ? container.querySelectorAll('tbody tr') : [];
        const countEl = document.getElementById('bankofcommerceViewerCount');
        const search = document.getElementById('bankofcommerceViewerSearch');
        const btn = document.getElementById('bankofcommerceViewerSearchBtn');
            ...
            if(q === ''){
                ...
                for(let i=0;i<rows.length;i++){
                    ...
                    if(VIEW_TYPE === 'web'){
                        ...
                        const NO_RECORD_ID = 'bankofcommerceNoRecord';
