...
// Mirrors mbtc-partnerdata.php — adapted for BDO.
...
// Ensure caller intended BDO processing
$company = isset($_POST['company']) ? (string) $_POST['company'] : '';
$bdoAliases = ['BDO', 'BDO UNIBANK', 'BDO UNIBANK INC.', 'BANCO DE ORO'];
if (!in_array(strtoupper(trim($company)), $bdoAliases, true)) {
    echo json_encode(['success' => false, 'error' => 'Partner extractor only allowed for BDO']);
    exit;
}
...
try {
    ...
    // Attempt to locate cover date (for headers like "For Period Covered [Feb 05 2026 to Feb 05 2026]")
    ...
    // Partner required columns
    $required = [
        'Date', 'Time', 'Reference No.', 'RTS Tracer No.', 'Provider',
        'Beneficiary Name', 'Remitter Name', 'PHP', 'USD', 'in PHP'
    ];

    // Search header row in first N rows instead of assuming row 1
    $headers = [];
    $headerRow = null;
    ...
    // Prefer stopping on Reference No., else Date
    ...
    for ($r = $headerRow + 1; $r <= $highestRow; $r++) {
        ...
        if ($stopByRef && $refColIdx !== null) {
            ...
            $refVal = $sheet->getCell($refColLetter . $r)->getValue();
            if ($refVal === null || trim((string) $refVal) === '') break;
        } elseif ($dateVal === null || trim((string) $dateVal) === '') {
            break;
        }
        ...
        // Only keep rows with a reference number
        $refValForRow = isset($item['Reference No.']) ? trim($item['Reference No.']) : '';
        if ($refValForRow === '') continue;
