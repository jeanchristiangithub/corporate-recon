...
// Mirrors mbtc-webdata.php — adapted for BDO (BDO UNIBANK)
...
try {
    ...
    // Helper: normalize header text (strip BOMs, NBSPs, collapse whitespace, uppercase)
    ...
    // Required columns — same web KPI format used by all ML corporate partners
    $required = [
    ...
    // Search the first 12 rows for a header row containing our required columns
    $headers   = [];
    $headerRow = null;
    ...
    for ($rowCheck = 1; $rowCheck <= $maxHeaderSearch; $rowCheck++) {
        ...
        // Treat as header row when at least 2 required columns are found
        if ($found >= 2) {
            $headers   = $tmp;
            $headerRow = $rowCheck;
            break;
    ...
    // Fallback to first row when nothing matched
    if ($headerRow === null) {
        for ($c = 1; $c <= $highestColIndex; $c++) {
            $colLetter = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($c);
    ...
    // Map header names to column index (case-insensitive, normalized)
    $map = [];
    ...
    for ($r = $headerRow + 1; $r <= $highestRow; $r++) {
        ...
        if ($ccref === '') {
            break; // Stopping condition: first empty CCREF NO ends the data range
        }

        // Capture date string from the first valid data row
        ...
        // Build row keyed by required column names
        $item = [];
