...
// Ensure caller intended BPIREMITTANCE processing
$company = isset($_POST['company']) ? (string) $_POST['company'] : '';
$bpiremittanceAliases = ['BPIREMITTANCE', 'BPI REMITTANCE', 'BPI-REMITTANCE'];
if(!in_array(strtoupper(trim($company)), $bpiremittanceAliases, true)){
    echo json_encode(['success'=>false,'error'=>'Partner extractor only allowed for BPIREMITTANCE']);
    exit;
}
...
try{
    ...
    // map headers with tolerant matching
    $map = [];
