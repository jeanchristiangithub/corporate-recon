<?php
// moneygram-helper.php
// Shared helpers for MONEYGRAM extraction, normalization and checks

require_once __DIR__ . '/../../../../vendor/autoload.php';
use PhpOffice\PhpSpreadsheet\Shared\Date as ExcelDate;

function moneygram_normalize_amount($val){
    $v = trim((string)$val);
    if($v === '') return null;
...
function moneygram_parse_date_claimed($raw){
...
function moneygram_build_pairs_from_payloads(array $payloads){
    $pairs = [];
    foreach($payloads as $pl){
        ...
        foreach($rows as $r){
            ...
            if($ccref !== ''){
                $pairs[] = ['ccref_no'=>$ccref, 'raw_date'=>$rawDate, 'date_claimed'=>moneygram_parse_date_claimed($rawDate)];
            }
        }
    }
    return $pairs;
...
function moneygram_normalize_pairs(array $pairs){
    $out = [];
    foreach($pairs as $p){
        $cc = isset($p['ccref_no']) ? trim((string)$p['ccref_no']) : '';
        $raw = $p['date_claimed'] ?? ($p['raw_date'] ?? ($p['date'] ?? ''));
        $norm = moneygram_parse_date_claimed($raw);
...
function moneygram_partner_normalize_currency($val){
    // similar to moneygram_normalize_amount but allow empty and zero
    $v = trim((string)$val);
    if($v === '') return null;
    $v = preg_replace('/[^0-9,\.\-() ]+/', '', $v);
    $neg = false;
...
function moneygram_partner_parse_date_time($dateRaw, $timeRaw = null){
...
function moneygram_partner_build_pairs_from_payloads(array $payloads){
    ...
    foreach($payloads as $pl){
        ...
        foreach($rows as $r){
            ...
            $rawTime = $r['Time'] ?? null;
            if($ref !== ''){
                $pairs[] = ['reference_no'=>$ref, 'raw_date'=>$rawDate, 'raw_time'=>$rawTime, 'date_time'=>moneygram_partner_parse_date_time($rawDate, $rawTime)];
            }
        }
    }
    return $pairs;
