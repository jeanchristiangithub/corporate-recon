...
try {
    ...
    if (isset($data['payloads']) && is_array($data['payloads'])) {
        $pairs = eec_build_pairs_from_payloads($data['payloads']);
        ...
        foreach ($data['pairs'] as $p) {
            ...
            $pairs[] = ['ccref_no' => $cc, 'raw_date' => $rawDate, 'date_claimed' => eec_parse_date_claimed($rawDate)];
        }
    }
