...
// Mirrors mbtc-partnerdata.php — adapted for EEC.
...
// Ensure caller intended EEC processing
$company = isset($_POST['company']) ? (string) $_POST['company'] : '';
$eecAliases = ['EEC', 'EEC/GOLDSTARINC', 'EEC GOLDSTAR'];
if (!in_array(strtoupper(trim($company)), $eecAliases, true)) {
    echo json_encode(['success' => false, 'error' => 'Partner extractor only allowed for EEC']);
    exit;
}
