<?php
// ezremit-helper.php
// Shared helpers for EZREMIT extraction, normalization and checks

require_once __DIR__ . '/../../../../vendor/autoload.php';
use PhpOffice\PhpSpreadsheet\Shared\Date as ExcelDate;

function ezremit_normalize_amount($val){
    $v = trim((string)$val);
    if($v === '') return null;
...
function ezremit_parse_date_claimed($raw){
...
function ezremit_build_pairs_from_payloads(array $payloads){
    $pairs = [];
    foreach($payloads as $pl){
        ...
        foreach($rows as $r){
            ...
            if($ccref !== ''){
                $pairs[] = ['ccref_no'=>$ccref, 'raw_date'=>$rawDate, 'date_claimed'=>ezremit_parse_date_claimed($rawDate)];
            }
        }
    }
    return $pairs;
...
function ezremit_normalize_pairs(array $pairs){
    $out = [];
    foreach($pairs as $p){
        $cc = isset($p['ccref_no']) ? trim((string)$p['ccref_no']) : '';
        $raw = $p['date_claimed'] ?? ($p['raw_date'] ?? ($p['date'] ?? ''));
        $norm = ezremit_parse_date_claimed($raw);
        $out[] = ['ccref_no'=>$cc, 'raw_date'=>$raw, 'normalized'=>$norm];
    }
    return $out;
}

function ezremit_partner_normalize_currency($val){
    $v = trim((string)$val);
    if($v === '') return null;
    $v = preg_replace('/[^0-9,\.\-() ]+/', '', $v);
    $neg = false;
...
function ezremit_partner_parse_date_time($dateRaw, $timeRaw = null){
...
function ezremit_partner_build_pairs_from_payloads(array $payloads){
    $pairs = [];
    foreach($payloads as $pl){
        $dateStr = $pl['dateStr'] ?? '';
        $rows = $pl['rows'] ?? [];
        foreach($rows as $r){
            ...
            $rawTime = $r['Time'] ?? null;
            if($ref !== ''){
                $pairs[] = ['reference_no'=>$ref, 'raw_date'=>$rawDate, 'raw_time'=>$rawTime, 'date_time'=>ezremit_partner_parse_date_time($rawDate, $rawTime)];
            }
        }
    }
    return $pairs;
