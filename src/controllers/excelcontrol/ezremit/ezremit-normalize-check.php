...
try{
    ...
    if(isset($data['payloads']) && is_array($data['payloads'])){
        $pairs = ezremit_build_pairs_from_payloads($data['payloads']);
        ...
        foreach($data['pairs'] as $p){
            ...
            $pairs[] = ['ccref_no'=>$cc, 'raw_date'=>$raw, 'date_claimed'=>ezremit_parse_date_claimed($raw)];
        }
    }
