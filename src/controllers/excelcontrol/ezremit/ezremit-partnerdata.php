...
// Ensure caller intended EZREMIT processing
$company = isset($_POST['company']) ? (string) $_POST['company'] : '';
$aliases = ['EZREMIT','EZ REMIT'];
if(!in_array(strtoupper(trim($company)), $aliases, true)){
    echo json_encode(['success'=>false,'error'=>'Partner extractor only allowed for EZREMIT']);
    exit;
}
