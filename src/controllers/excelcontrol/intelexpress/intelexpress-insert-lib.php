...
class IntelexpressInsert {
    ...
    public function insertWebData(string $company, array $payloads): array{
        ...
        $sql = 'INSERT INTO intelexpress_web_data (partnerName, `no`, control_series_no, date_claimed, kptn, ccref_no, currency, amount, ctc, ctp, sender_name, sender_country, beneficiary_receiver, receiver_kyc, receiver_phone, operator, branch, remote_operator, remote_branch, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
        ...
        foreach ($payloads as $pl) {
            ...
            foreach ($rows as $r) {
                ...
                $stmt->execute([
                    ...
                    $r['CCREF NO'] ?? '',
                    $r['CURRENCY'] ?? '',
                    intelexpress_normalize_amount($r['AMOUNT'] ?? ''),
                    $r['CTC'] ?? '',
                    $r['CTP'] ?? '',
                    $r['SENDER NAME'] ?? '',
                    $r['SENDER COUNTRY'] ?? '',
