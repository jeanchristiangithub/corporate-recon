<?php
// intelexpress-insert.php

require_once __DIR__ . '/intelexpress-insert-lib.php';
...
try {
    ...
    if ($action === 'check') {
        ...
        foreach ($pairs as $p) {
            ...
            if ($cc === '') continue;
            $norm = intelexpress_parse_date_claimed($rawDate);
            if ($norm !== null) {
                $stmt = $pdo->prepare('SELECT ccref_no, date_claimed, COUNT(*) as cnt FROM intelexpress_web_data WHERE ccref_no = ? AND date_claimed = ? GROUP BY ccref_no, date_claimed');
            ...
            if ($ts !== false) {
                ...
                $stmt2 = $pdo->prepare('SELECT ccref_no, date_claimed, COUNT(*) as cnt FROM intelexpress_web_data WHERE ccref_no = ? AND DATE(date_claimed) = ? GROUP BY ccref_no, date_claimed');
            ...
            $stmt3 = $pdo->prepare('SELECT ccref_no, date_claimed, COUNT(*) as cnt FROM intelexpress_web_data WHERE ccref_no = ? GROUP BY ccref_no, date_claimed');
            $stmt3->execute([$cc]);
            $r3 = $stmt3->fetchAll();
            foreach ($r3 as $ra) {
                $key = $ra['ccref_no'] . '|' . $ra['date_claimed'];
    ...
    if ($action === 'delete') {
        ...
        foreach (array_chunk($pairs, 5000) as $chunk) {
            ...
            $sql = 'DELETE FROM intelexpress_web_data WHERE (ccref_no, date_claimed) IN (' . implode(',', $place) . ')';
    ...
    if ($action === 'insert_web') {
        $company = isset($data['company']) ? $data['company'] : 'IntelExpress';
        $payloads = isset($data['payloads']) && is_array($data['payloads']) ? $data['payloads'] : [];
        $ins = new IntelexpressInsert();
