<?php
// intelexpress-normalize-check.php

require_once __DIR__ . '/../../../config/db.php';
require_once __DIR__ . '/intelexpress-helper.php';
...
try {
    ...
    if (isset($data['payloads']) && is_array($data['payloads'])) {
        $pairs = intelexpress_build_pairs_from_payloads($data['payloads']);
    } elseif (isset($data['pairs']) && is_array($data['pairs'])) {
        foreach ($data['pairs'] as $p) {
            $cc = isset($p['ccref_no']) ? trim((string) $p['ccref_no']) : '';
            $rawDate = $p['date_claimed'] ?? ($p['raw_date'] ?? '');
            $pairs[] = ['ccref_no' => $cc, 'raw_date' => $rawDate, 'date_claimed' => intelexpress_parse_date_claimed($rawDate)];
        }
    }
