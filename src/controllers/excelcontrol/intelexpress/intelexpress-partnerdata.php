...
// Ensure caller intended IntelExpress processing
$company = isset($_POST['company']) ? (string) $_POST['company'] : '';
$intelexpressAliases = ['INTELEXPRESS', 'INTEL EXPRESS'];
if (!in_array(strtoupper(trim($company)), $intelexpressAliases, true)) {
    echo json_encode(['success' => false, 'error' => 'Partner extractor only allowed for IntelExpress']);
    exit;
}
