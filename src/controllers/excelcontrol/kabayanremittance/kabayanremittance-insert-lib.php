<?php
// kabayanremittance-insert-lib.php
// Reusable insertion helper for KabayanRemittance web and partner payloads

require_once __DIR__ . '/../../../config/db.php';
require_once __DIR__ . '/kabayanremittance-helper.php';
require_once __DIR__ . '/../../../../vendor/autoload.php';
use PhpOffice\PhpSpreadsheet\Shared\Date as ExcelDate;

class KabayanRemittanceInsert {
    private $pdo;
    ...
    public function insertWebData(string $company, array $payloads): array{
        ...
        $sql = 'INSERT INTO kabayanremittance_web_data (partnerName, `no`, control_series_no, date_claimed, kptn, ccref_no, currency, amount, ctc, ctp, sender_name, sender_country, beneficiary_receiver, receiver_kyc, receiver_phone, operator, branch, remote_operator, remote_branch, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
        ...
        foreach($payloads as $pl){
            ...
            foreach($rows as $r){
                ...
                $ccref = $r['CCREF NO'] ?? '';
                $currency = $r['CURRENCY'] ?? '';
                $amount = kabayanremittance_normalize_amount($r['AMOUNT'] ?? '');
                $ctc = $r['CTC'] ?? '';
                $ctp = $r['CTP'] ?? '';
    ...
    public function insertPartnerData(string $company, array $payloads): array{
        ...
        $now = date('Y-m-d H:i:s');
        $inserted = 0;
        $sql = 'INSERT INTO kabayanremittance_partner_data (partnerName, `date`, cover_date, `time`, reference_no, rts_tracer_no, provider, beneficiary_name, remitter_name, `php`, `usd`, in_php, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
        $stmt = $pdo->prepare($sql);
        ...
        foreach($payloads as $pl){
            ...
            foreach($rows as $r){
                ...
                $normalized = kabayanremittance_partner_parse_date_time($dateRaw, $timeRaw);
                if($normalized !== null){
                ...
                $php = kabayanremittance_partner_normalize_currency($phpRaw);
                $usd = kabayanremittance_partner_normalize_currency($usdRaw);
                $in_php = kabayanremittance_partner_normalize_currency($inPhpRaw);
