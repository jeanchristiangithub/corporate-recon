<?php
// mbtc-helper.php
// Shared helpers for MBTC extraction, normalization and checks

require_once __DIR__ . '/../../../../vendor/autoload.php';
use PhpOffice\PhpSpreadsheet\Shared\Date as ExcelDate;

function mbtc_normalize_amount($val){
    $v = trim((string)$val);
    if($v === '') return null;
...
function mbtc_parse_date_claimed($raw){
...
function mbtc_build_pairs_from_payloads(array $payloads){
    $pairs = [];
    foreach($payloads as $pl){
        ...
        foreach($rows as $r){
            ...
            if($ccref !== ''){
                $pairs[] = ['ccref_no'=>$ccref, 'raw_date'=>$rawDate, 'date_claimed'=>mbtc_parse_date_claimed($rawDate)];
            }
        }
    }
    return $pairs;
...
function mbtc_partner_normalize_currency($val){
    // similar to mbtc_normalize_amount but allow empty and zero
    $v = trim((string)$val);
    if($v === '') return null;
    $v = preg_replace('/[^0-9,\.\-() ]+/', '', $v);
    $neg = false;
...
function mbtc_partner_parse_date_time($dateRaw, $timeRaw = null){
...
function mbtc_partner_build_pairs_from_payloads(array $payloads){
    ...
    foreach($payloads as $pl){
        ...
        foreach($rows as $r){
            ...
            $rawTime = $r['Time'] ?? null;
            if($ref !== ''){
                $pairs[] = ['reference_no'=>$ref, 'raw_date'=>$rawDate, 'raw_time'=>$rawTime, 'date_time'=>mbtc_partner_parse_date_time($rawDate, $rawTime)];
            }
        }
    }
    return $pairs;
