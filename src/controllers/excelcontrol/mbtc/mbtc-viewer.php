...
.mbtc-viewer{padding:14px 10px;color:#1f2937}
.mbtc-viewer h3{display:block;margin:0 0 6px 0;color:#1f2937}
.mbtc-view-meta{display:flex;gap:12px;align-items:center;margin-bottom:10px}
.mbtc-viewer #mbtcViewerContainer{overflow:auto;max-height:78vh}
.mbtc-view-table{border-collapse:collapse;width:auto;min-width:1400px;table-layout:fixed;font-size:13px;border-spacing:0}
.mbtc-view-table th,
.mbtc-view-table td{
...
.mbtc-view-table th:first-child,
.mbtc-view-table td:first-child{
    position:sticky;
    left:0;
    z-index:7;
    background:#fff;
}
.mbtc-view-table tbody tr:nth-child(even) td{background:rgba(240,244,247,0.6)}
.mbtc-view-table tbody tr.search-match td{ background: #fff7ed; }
.mbtc-viewer input#mbtcViewerSearch{padding:8px 10px;border-radius:8px;border:1px solid #e5e7eb}
.mbtc-viewer .viewer-title{font-weight:600;color:#1f2937}
/* Reserve space on the right of the header for a modal close button */
.mbtc-recon-modal__head{padding-right:56px}
@media (max-width:720px){
    .mbtc-recon-modal__head{padding-right:12px}
    .mbtc-viewer input#mbtcViewerSearch{max-width:180px}
}
</style>
<div class="mbtc-viewer" style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;">
    <div class="mbtc-recon-modal__head" style="border-bottom:0;margin-bottom:8px;padding-bottom:6px;display:flex;align-items:center;justify-content:space-between">
        <div>
            <h3 style="margin:0;font-size:1.05rem;color:#1f2937;font-weight:700;">
                <?php echo (($partnerName && $partnerName !== '') ? $partnerName : 'MBTC') . ' - ' . $dateStr; ?>
            </h3>
            ...
            <div style="font-size:0.95rem;color:#666;flex:0 0 auto">Rows: <strong id="mbtcViewerCount"><?php echo number_format(count($rows)); ?></strong></div>
            <input id="mbtcViewerSearch" placeholder="Search" style="flex:1;min-width:0;max-width:360px;padding:8px 10px;border:1px solid #ddd;border-radius:8px" />
            <button id="mbtcViewerSearchBtn" class="material-btn" style="white-space:nowrap">Search</button>
        </div>
    </div>
    <div style="overflow:auto; max-height:78vh;" id="mbtcViewerContainer">
        <table id="mbtcViewerTable" class="mbtc-view-table" border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:auto;min-width:1400px">
            <thead>
                <tr>
<?php
...
if(empty($headers)){
    ...
    // known label maps
    $partnerMap = [ 'date'=>'Date','time'=>'Time','reference_no'=>'Reference No.','rts_tracer_no'=>'RTS Tracer No.','provider'=>'Provider','beneficiary_name'=>'Beneficiary Name','remitter_name'=>'Remitter Name','php'=>'Payout Amount PHP','usd'=>'USD','in_php'=>'in PHP','cover_date'=>'Cover Date','partnerName'=>'Partner' ];
...
<?php
    ...
    ;(function(){
        ...
        const container = document.getElementById('mbtcViewerContainer');
        const table = document.getElementById('mbtcViewerTable');
        const rows = container ? container.querySelectorAll('tbody tr') : [];
        const countEl = document.getElementById('mbtcViewerCount');
        const search = document.getElementById('mbtcViewerSearch');
        const btn = document.getElementById('mbtcViewerSearchBtn');
            ...
            if(q === ''){
                ...
                for(let i=0;i<rows.length;i++){
                    ...
                    if(VIEW_TYPE === 'web'){
                        ...
                        const NO_RECORD_ID = 'mbtcNoRecord';
