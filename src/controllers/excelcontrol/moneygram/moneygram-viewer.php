...
function moneygram_base_col_key($key){
    $k = strtolower(trim((string)$key));
    if(strpos($k, 'web_') === 0) $k = substr($k, 4);
    if(strpos($k, 'partner_') === 0) $k = substr($k, 8);
    return $k;
}

function moneygram_is_money_column($key, $label = ''){
    $base = moneygram_base_col_key($key);
    $baseNorm = str_replace([' ', '-', '/'], '_', strtolower(trim($base)));
    $labelNorm = strtolower(trim((string)$label));

    $exact = [
        'amount','charge','ctc','ctp','principal','commission','total','php','usd','in_php',
        'base_tran_amt','fee_tran_amt','fx_rev_share_tran_amt','comm_tran_amt','total_tran_amt','payout_amount_php'
    ];
    if(in_array($baseNorm, $exact, true)) return true;

    $keywordHits = ['amount','charge','ctc','ctp','principal','commission','total','payout amount'];
    foreach($keywordHits as $kw){
        if(strpos($baseNorm, str_replace(' ', '_', $kw)) !== false) return true;
        if($labelNorm !== '' && strpos($labelNorm, $kw) !== false) return true;
    }

    if(($baseNorm === 'php' || $baseNorm === 'usd') && strpos($labelNorm, 'count') === false) return true;
    return false;
}

function moneygram_format_money_display($value){
    ...
    if($clean === '' || $clean === '-' || $clean === '.' || $clean === ',') return '0.00';
...
.moneygram-view-table th,
.moneygram-view-table td{
...
.moneygram-viewer .viewer-title{font-weight:600;color:#1f2937}
/* Reserve space on the right of the header for a modal close button */
.moneygram-recon-modal__head{padding-right:56px}
@media (max-width:720px){
    .moneygram-recon-modal__head{padding-right:12px}
    .moneygram-viewer input#moneygramViewerSearch{max-width:180px}
}
</style>
<div class="moneygram-viewer" style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;">
    ...
    <div style="overflow:auto; max-height:78vh;" id="moneygramViewerContainer">
        <table id="moneygramViewerTable" class="moneygram-view-table" border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:auto;min-width:1400px">
            <thead>
                <tr>
<?php
...
if(empty($headers)){
    ...
    foreach($display as $hk => $orig){
        ...
        $moneyColumns[$hk] = moneygram_is_money_column($hk, $label);
...
foreach($rows as $r){
    ...
    if(empty($display)){
        ...
        foreach(array_keys($display) as $hk){
            ...
            if(isset($moneyColumns[$hk]) && $moneyColumns[$hk] === true){
                $v = moneygram_format_money_display($v);
            }
...
<?php
    ...
    ;(function(){
        ...
        const container = document.getElementById('moneygramViewerContainer');
        const table = document.getElementById('moneygramViewerTable');
        const rows = container ? container.querySelectorAll('tbody tr') : [];
        const countEl = document.getElementById('moneygramViewerCount');
        const search = document.getElementById('moneygramViewerSearch');
        const btn = document.getElementById('moneygramViewerSearchBtn');
            ...
            if(q === ''){
                ...
                for(let i=0;i<rows.length;i++){
                    ...
                    if(VIEW_TYPE === 'web'){
                        ...
                        const NO_RECORD_ID = 'moneygramNoRecord';
