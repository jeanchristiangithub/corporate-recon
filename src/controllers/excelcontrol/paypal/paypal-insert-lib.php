<?php
// paypal-insert-lib.php
// Reusable insertion helper for PAYPAL web and partner payloads
...
class PaypalInsert {
	...
	public function insertWebData(string $company, array $payloads): array{
		...
		$sql = 'INSERT INTO paypal_web_data (partnerName, `no`, control_series_no, date_claimed, kptn, ccref_no, currency, amount, ctc, ctp, sender_name, sender_country, beneficiary_receiver, receiver_kyc, receiver_phone, operator, branch, remote_operator, remote_branch, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
		...
		foreach($payloads as $pl){
			...
			foreach($rows as $r){
				...
				$ccref = $r['CCREF NO'] ?? '';
				$currency = $r['CURRENCY'] ?? '';
				$amount = paypal_normalize_amount($r['AMOUNT'] ?? '');
				$ctc = $r['CTC'] ?? '';
				$ctp = $r['CTP'] ?? '';
	...
	public function insertPartnerData(string $company, array $payloads): array{
		...
		$now = date('Y-m-d H:i:s');
		$inserted = 0;
		$sql = 'INSERT INTO paypal_partner_data (partnerName, `date`, cover_date, `time`, reference_no, rts_tracer_no, provider, beneficiary_name, remitter_name, `php`, `usd`, in_php, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
		$stmt = $pdo->prepare($sql);
		...
		foreach($payloads as $pl){
			...
			foreach($rows as $r){
				...
				$normalized = paypal_partner_parse_date_time($dateRaw, $timeRaw);
				if($normalized !== null){
				...
				$php = paypal_partner_normalize_currency($phpRaw);
				$usd = paypal_partner_normalize_currency($usdRaw);
				$in_php = paypal_partner_normalize_currency($inPhpRaw);
