...
// Ensure caller intended PayPal processing
$company = isset($_POST['company']) ? (string) $_POST['company'] : '';
$paypalAliases = ['PAYPAL', 'PAYPAL INC', 'PAYPAL PH'];
if(!in_array(strtoupper(trim($company)), $paypalAliases, true)){
	echo json_encode(['success'=>false,'error'=>'Partner extractor only allowed for PAYPAL']);
	exit;
}
