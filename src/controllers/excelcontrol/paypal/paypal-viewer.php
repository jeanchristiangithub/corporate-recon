...
.paypal-viewer{padding:14px 10px;color:#1f2937}
.paypal-viewer h3{display:block;margin:0 0 6px 0;color:#1f2937}
.paypal-view-meta{display:flex;gap:12px;align-items:center;margin-bottom:10px}
.paypal-viewer #paypalViewerContainer{overflow:auto;max-height:78vh}
.paypal-view-table{border-collapse:collapse;width:auto;min-width:1400px;table-layout:fixed;font-size:13px;border-spacing:0}
.paypal-view-table th,
.paypal-view-table td{
...
.paypal-view-table th:first-child,
.paypal-view-table td:first-child{
	position:sticky;
	left:0;
	z-index:7;
	background:#fff;
}
.paypal-view-table tbody tr:nth-child(even) td{background:rgba(240,244,247,0.6)}
.paypal-view-table tbody tr.search-match td{ background: #fff7ed; }
.paypal-viewer input#paypalViewerSearch{padding:8px 10px;border-radius:8px;border:1px solid #e5e7eb}
.paypal-viewer .viewer-title{font-weight:600;color:#1f2937}
</style>
<div class="paypal-viewer" style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;">
	<div class="paypal-recon-modal__head" style="border-bottom:0;margin-bottom:8px;padding-bottom:6px;display:flex;align-items:center;justify-content:space-between">
		<div>
			<h3 style="margin:0;font-size:1.05rem;color:#1f2937;font-weight:700;">
				<?php echo (($partnerName && $partnerName !== '') ? $partnerName : 'PAYPAL') . ' - ' . $dateStr; ?>
			</h3>
			...
			<div style="font-size:0.95rem;color:#666">Rows: <strong id="paypalViewerCount"><?php echo number_format(count($rows)); ?></strong></div>
			<input id="paypalViewerSearch" placeholder="Search" style="padding:8px 10px;border:1px solid #ddd;border-radius:8px;width:280px" />
			<button id="paypalViewerSearchBtn" class="material-btn">Search</button>
		</div>
	</div>
	<div style="overflow:auto; max-height:78vh;" id="paypalViewerContainer">
		<table id="paypalViewerTable" class="paypal-view-table" border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:auto;min-width:1400px">
			<thead>
				<tr>
<?php
...
<?php
	...
	;(function(){
		...
		const container = document.getElementById('paypalViewerContainer');
		const table = document.getElementById('paypalViewerTable');
		const rows = container ? container.querySelectorAll('tbody tr') : [];
		const countEl = document.getElementById('paypalViewerCount');
		const search = document.getElementById('paypalViewerSearch');
		const btn = document.getElementById('paypalViewerSearchBtn');
			...
			if(q === ''){
				...
				for(let i=0;i<rows.length;i++){
					...
					if(VIEW_TYPE === 'web'){
						...
						const NO_RECORD_ID = 'paypalNoRecord';
