...
// Ensure caller intended PLACIDEXPRESS processing
$company = isset($_POST['company']) ? (string) $_POST['company'] : '';
$aliases = ['PLACIDEXPRESS', 'PLACID EXPRESS', 'PLACIDEXPRESS CORPORATE'];
if(!in_array(strtoupper(trim($company)), $aliases, true)){
    echo json_encode(['success'=>false,'error'=>'Partner extractor only allowed for PLACIDEXPRESS']);
    exit;
}
