<?php
// riafinancials-insert.php
// Handles duplicate checks, deletes and inserts for RIA Financials upload flow

require_once __DIR__ . '/riafinancials-insert-lib.php';
...
try{
	...
	if($action === 'check'){
		...
		foreach($pairs as $p){
			...
			$norm = riafinancials_parse_date_claimed($rawDate);
			if($norm !== null){
	...
	if($action === 'delete'){
		...
		foreach(array_chunk($pairs, 5000) as $chunk){
			...
			$sql = 'DELETE FROM riafinancials_web_data WHERE (ccref_no, date_claimed) IN (' . implode(',', $place) . ')';
	...
	if($action === 'check_partner'){
		...
		foreach($pairs as $p){
			...
			if($dateOnly !== null){
				$stmt = $pdo->prepare('SELECT reference_no, date, COUNT(*) as cnt FROM riafinancials_partner_data WHERE reference_no = ? AND DATE(`date`) = ? GROUP BY reference_no, date');
			...
			$stmt3 = $pdo->prepare('SELECT reference_no, date, COUNT(*) as cnt FROM riafinancials_partner_data WHERE reference_no = ? GROUP BY reference_no, date');
			$stmt3->execute([$ref]);
			$r3 = $stmt3->fetchAll();
			foreach($r3 as $ra){
				if(isset($ra['cnt']) && (int)$ra['cnt'] > 0){
	...
	if($action === 'delete_partner'){
		...
		foreach(array_chunk($pairs, 5000) as $chunk){
			...
			$sql = 'DELETE FROM riafinancials_partner_data WHERE (reference_no, date) IN (' . implode(',', $place) . ')';
			$stmt = $pdo->prepare($sql);
			$stmt->execute($params);
			$cnt += $stmt->rowCount();
		}
	...
	if($action === 'insert_web'){
		$company = isset($data['company']) ? $data['company'] : 'RIA Financials';
		$payloads = isset($data['payloads']) && is_array($data['payloads']) ? $data['payloads'] : [];
		$ins = new RiafinancialsInsert();
