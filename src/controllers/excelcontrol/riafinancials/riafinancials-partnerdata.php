...
// Ensure caller intended RIA Financials processing
$company = isset($_POST['company']) ? (string) $_POST['company'] : '';
$riaAliases = ['RIA FINANCIALS', 'RIAFINANCIALS'];
if(!in_array(strtoupper(trim($company)), $riaAliases, true)){
	echo json_encode(['success'=>false,'error'=>'Partner extractor only allowed for RIA Financials']);
	exit;
}
...
try{
	...
	for($r=$headerRow+1;$r<=$highestRow;$r++){
		...
		$refValForRow = isset($item['Reference No.']) ? trim((string)$item['Reference No.']) : '';
		if($refValForRow === ''){
			continue;
		}
