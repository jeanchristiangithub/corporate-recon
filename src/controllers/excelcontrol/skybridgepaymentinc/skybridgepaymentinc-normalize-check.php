<?php
// skybridgepaymentinc-normalize-check.php
...
try{
    ...
    if($action === 'normalize_pairs'){
        ...
        $out = skybridgepaymentinc_normalize_pairs($pairs);
    ...
    if($action === 'build_pairs_web'){
        ...
        $pairs = skybridgepaymentinc_build_pairs_from_payloads($payloads);
        echo json_encode(['success'=>true,'pairs'=>$pairs]); exit;
    }
    if($action === 'build_pairs_partner'){
        $payloads = isset($data['payloads']) && is_array($data['payloads']) ? $data['payloads'] : [];
        $pairs = skybridgepaymentinc_partner_build_pairs_from_payloads($payloads);
        echo json_encode(['success'=>true,'pairs'=>$pairs]); exit;
    }
