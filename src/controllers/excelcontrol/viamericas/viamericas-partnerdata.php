...
// Ensure caller intended VIAMERICAS processing
$company = isset($_POST['company']) ? (string) $_POST['company'] : '';
$viamericasAliases = ['VIAMERICAS', 'VIA AMERICAS', 'VIAAMERICAS'];
if(!in_array(strtoupper(trim($company)), $viamericasAliases, true)){
    echo json_encode(['success'=>false,'error'=>'Partner extractor only allowed for VIAMERICAS']);
    exit;
}
