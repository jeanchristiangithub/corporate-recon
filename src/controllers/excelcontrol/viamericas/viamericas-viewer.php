...
.viamericas-viewer{padding:14px 10px;color:#1f2937}
.viamericas-viewer h3{display:block;margin:0 0 6px 0;color:#1f2937}
.viamericas-view-meta{display:flex;gap:12px;align-items:center;margin-bottom:10px}
.viamericas-viewer #viamericasViewerContainer{overflow:auto;max-height:78vh}
.viamericas-view-table{border-collapse:collapse;width:auto;min-width:1400px;table-layout:fixed;font-size:13px;border-spacing:0}
.viamericas-view-table th,
.viamericas-view-table td{
...
.viamericas-view-table thead th{
    position:sticky;
    top:0;
    z-index:6;
    background:#f9fafb;
...
.viamericas-view-table th:first-child,
.viamericas-view-table td:first-child{
    position:sticky;
    left:0;
    z-index:7;
    background:#fff;
}
.viamericas-view-table tbody tr:nth-child(even) td{background:rgba(240,244,247,0.6)}
.viamericas-view-table tbody tr.search-match td{ background: #fff7ed; }
.viamericas-viewer input#viamericasViewerSearch{padding:8px 10px;border-radius:8px;border:1px solid #e5e7eb}
.viamericas-viewer .viewer-title{font-weight:600;color:#1f2937}
.viamericas-recon-modal__head{padding-right:56px}
@media (max-width:720px){
    .viamericas-recon-modal__head{padding-right:12px}
    .viamericas-viewer input#viamericasViewerSearch{max-width:180px}
}
</style>
<div class="viamericas-viewer" style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;">
    <div class="viamericas-recon-modal__head" style="border-bottom:0;margin-bottom:8px;padding-bottom:6px;display:flex;align-items:center;justify-content:space-between">
        <div>
            <h3 style="margin:0;font-size:1.05rem;color:#1f2937;font-weight:700;">
                <?php echo (($partnerName && $partnerName !== '') ? $partnerName : 'VIAMERICAS') . ' - ' . $dateStr; ?>
            </h3>
            ...
            <div style="font-size:0.95rem;color:#666;flex:0 0 auto">Rows: <strong id="viamericasViewerCount"><?php echo number_format(count($rows)); ?></strong></div>
            <input id="viamericasViewerSearch" placeholder="Search" style="flex:1;min-width:0;max-width:360px;padding:8px 10px;border:1px solid #ddd;border-radius:8px" />
            <button id="viamericasViewerSearchBtn" class="material-btn" style="white-space:nowrap">Search</button>
        </div>
    </div>
    <div style="overflow:auto; max-height:78vh;" id="viamericasViewerContainer">
        <table id="viamericasViewerTable" class="viamericas-view-table" border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:auto;min-width:1400px">
            <thead>
                <tr>
<?php
$headers = [];
...
<?php
    ...
    ;(function(){
        ...
        const container = document.getElementById('viamericasViewerContainer');
        const table = document.getElementById('viamericasViewerTable');
        const rows = container ? container.querySelectorAll('tbody tr') : [];
        const countEl = document.getElementById('viamericasViewerCount');
        const search = document.getElementById('viamericasViewerSearch');
        const btn = document.getElementById('viamericasViewerSearchBtn');
        ...
        function doFilter(){
            ...
            if(q === ''){
                ...
                for(let i=0;i<rows.length;i++){
                    ...
                    let key = '';
                    if(VIEW_TYPE === 'web'){
                        const NO_RECORD_ID = 'viamericasNoRecord';
