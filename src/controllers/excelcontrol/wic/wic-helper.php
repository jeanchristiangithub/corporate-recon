<?php
// wic-helper.php
// Shared helpers for WIC extraction, normalization and checks

require_once __DIR__ . '/../../../../vendor/autoload.php';
use PhpOffice\PhpSpreadsheet\Shared\Date as ExcelDate;

function wic_normalize_amount($val){
    $v = trim((string)$val);
    if($v === '') return null;
...
function wic_parse_date_claimed($raw){
...
function wic_build_pairs_from_payloads(array $payloads){
    $pairs = [];
    foreach($payloads as $pl){
        ...
        foreach($rows as $r){
            ...
            if($ccref !== ''){
                $pairs[] = ['ccref_no'=>$ccref, 'raw_date'=>$rawDate, 'date_claimed'=>wic_parse_date_claimed($rawDate)];
            }
        }
    }
    return $pairs;
...
function wic_partner_parse_date_time($dateRaw, $timeRaw = null){
...
function wic_partner_build_pairs_from_payloads(array $payloads){
    $pairs = [];
    foreach($payloads as $pl){
        $dateStr = $pl['dateStr'] ?? '';
        $rows = $pl['rows'] ?? [];
        foreach($rows as $r){
            ...
            $rawTime = $r['Time'] ?? null;
            if($ref !== ''){
                $pairs[] = ['reference_no'=>$ref, 'raw_date'=>$rawDate, 'raw_time'=>$rawTime, 'date_time'=>wic_partner_parse_date_time($rawDate, $rawTime)];
            }
        }
    }
    return $pairs;
