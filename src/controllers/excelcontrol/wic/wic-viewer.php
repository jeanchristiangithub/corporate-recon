...
?>
<style>
.wic-viewer{padding:14px 10px;color:#1f2937}
.wic-viewer h3{display:block;margin:0 0 6px 0;color:#1f2937}
.wic-view-meta{display:flex;gap:12px;align-items:center;margin-bottom:10px}
.wic-viewer #wicViewerContainer{overflow:auto;max-height:78vh}
.wic-view-table{border-collapse:collapse;width:auto;min-width:1400px;table-layout:fixed;font-size:13px;border-spacing:0}
.wic-view-table th,
.wic-view-table td{
...
.wic-view-table th:first-child,
.wic-view-table td:first-child{
    position:sticky;
    left:0;
    z-index:7;
    background:#fff;
}
.wic-view-table tbody tr:nth-child(even) td{background:rgba(240,244,247,0.6)}
.wic-view-table tbody tr.search-match td{ background: #fff7ed; }
.wic-viewer input#wicViewerSearch{padding:8px 10px;border-radius:8px;border:1px solid #e5e7eb}
.wic-viewer .viewer-title{font-weight:600;color:#1f2937}
/* Reserve space on the right of the header for a modal close button */
.wic-recon-modal__head{padding-right:56px}
@media (max-width:720px){
    .wic-recon-modal__head{padding-right:12px}
    .wic-viewer input#wicViewerSearch{max-width:180px}
}
</style>
<div class="wic-viewer" style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;">
    <div class="wic-recon-modal__head" style="border-bottom:0;margin-bottom:8px;padding-bottom:6px;display:flex;align-items:center;justify-content:space-between">
        <div>
            <h3 style="margin:0;font-size:1.05rem;color:#1f2937;font-weight:700;">
                <?php echo (($partnerName && $partnerName !== '') ? $partnerName : 'WIC') . ' - ' . $dateStr; ?>
            </h3>
           ...
           <div style="display:flex;align-items:center;gap:12px;min-width:0">
            <div style="font-size:0.95rem;color:#666;flex:0 0 auto">Rows: <strong id="wicViewerCount"><?php echo number_format(count($rows)); ?></strong></div>
            <input id="wicViewerSearch" placeholder="Search" style="flex:1;min-width:0;max-width:360px;padding:8px 10px;border:1px solid #ddd;border-radius:8px" />
            <button id="wicViewerSearchBtn" class="material-btn" style="white-space:nowrap">Search</button>
        </div>
    </div>
    <div style="overflow:auto; max-height:78vh;" id="wicViewerContainer">
        <table id="wicViewerTable" class="wic-view-table" border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:auto;min-width:1400px">
            <thead>
                <tr>
<?php
$headers = [];
...
<?php
    ...
    ;(function(){
        ...
        const container = document.getElementById('wicViewerContainer');
        const table = document.getElementById('wicViewerTable');
        const rows = container ? container.querySelectorAll('tbody tr') : [];
        const countEl = document.getElementById('wicViewerCount');
        const search = document.getElementById('wicViewerSearch');
        const btn = document.getElementById('wicViewerSearchBtn');
        ...
        function doFilter(){
            ...
            if(q === ''){
                ...
                for(let i=0;i<rows.length;i++){
                    ...
                    let key = '';
                    if(VIEW_TYPE === 'web'){
                        const NO_RECORD_ID = 'wicNoRecord';
