...
.worldremitlimited-viewer{padding:14px 10px;color:#1f2937}
.worldremitlimited-viewer h3{display:block;margin:0 0 6px 0;color:#1f2937}
.worldremitlimited-view-meta{display:flex;gap:12px;align-items:center;margin-bottom:10px}
.worldremitlimited-viewer #worldremitlimitedViewerContainer{overflow:auto;max-height:78vh}
.worldremitlimited-view-table{border-collapse:collapse;width:auto;min-width:1400px;table-layout:fixed;font-size:13px;border-spacing:0}
.worldremitlimited-view-table th,
.worldremitlimited-view-table td{
...
.worldremitlimited-view-table thead th{
    position:sticky;
    top:0;
    z-index:6;
    background:#f9fafb;
...
.worldremitlimited-view-table th:first-child,
.worldremitlimited-view-table td:first-child{
    position:sticky;
    left:0;
    z-index:7;
    background:#fff;
}
.worldremitlimited-view-table tbody tr:nth-child(even) td{background:rgba(240,244,247,0.6)}
.worldremitlimited-view-table tbody tr.search-match td{ background: #fff7ed; }
.worldremitlimited-viewer input#worldremitlimitedViewerSearch{padding:8px 10px;border-radius:8px;border:1px solid #e5e7eb}
.worldremitlimited-viewer .viewer-title{font-weight:600;color:#1f2937}
.worldremitlimited-recon-modal__head{padding-right:56px}
@media (max-width:720px){
    .worldremitlimited-recon-modal__head{padding-right:12px}
    .worldremitlimited-viewer input#worldremitlimitedViewerSearch{max-width:180px}
}
</style>
<div class="worldremitlimited-viewer" style="font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial;">
    <div class="worldremitlimited-recon-modal__head" style="border-bottom:0;margin-bottom:8px;padding-bottom:6px;display:flex;align-items:center;justify-content:space-between">
        <div>
            <h3 style="margin:0;font-size:1.05rem;color:#1f2937;font-weight:700;">
                <?php echo (($partnerName && $partnerName !== '') ? $partnerName : 'WORLDREMITLIMITED') . ' - ' . $dateStr; ?>
            </h3>
            ...
            <div style="font-size:0.95rem;color:#666;flex:0 0 auto">Rows: <strong id="worldremitlimitedViewerCount"><?php echo number_format(count($rows)); ?></strong></div>
            <input id="worldremitlimitedViewerSearch" placeholder="Search" style="flex:1;min-width:0;max-width:360px;padding:8px 10px;border:1px solid #ddd;border-radius:8px" />
            <button id="worldremitlimitedViewerSearchBtn" class="material-btn" style="white-space:nowrap">Search</button>
        </div>
    </div>
    <div style="overflow:auto; max-height:78vh;" id="worldremitlimitedViewerContainer">
        <table id="worldremitlimitedViewerTable" class="worldremitlimited-view-table" border="1" cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:auto;min-width:1400px">
            <thead>
                <tr>
<?php
$headers = [];
...
<?php
    ...
    ;(function(){
        ...
        const container = document.getElementById('worldremitlimitedViewerContainer');
        const table = document.getElementById('worldremitlimitedViewerTable');
        const rows = container ? container.querySelectorAll('tbody tr') : [];
        const countEl = document.getElementById('worldremitlimitedViewerCount');
        const search = document.getElementById('worldremitlimitedViewerSearch');
        const btn = document.getElementById('worldremitlimitedViewerSearchBtn');
        ...
        function doFilter(){
            ...
            if(q === ''){
                ...
                for(let i=0;i<rows.length;i++){
                    ...
                    let key = '';
                    if(VIEW_TYPE === 'web'){
                        const NO_RECORD_ID = 'worldremitlimitedNoRecord';
