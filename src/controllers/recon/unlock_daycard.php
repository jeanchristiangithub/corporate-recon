...
try {
    ...
    $sql = "INSERT INTO recon_daycard_locks (
                ...
                :partner, :recon_date, 0, NULL, NULL, :unlocked_by, NOW()
            )
    ...
    $stmt->execute([
        ':partner' => $partner,
        ':recon_date' => $date,
        ':unlocked_by' => reconDaycardLocksUsername(),
    ]);

    echo json_encode(['success' => true, 'partner' => $partner, 'date' => $date, 'is_locked' => 0]);
    exit;
} catch (Throwable $e) {
